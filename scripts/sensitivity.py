...
def discriminant_check(config: dict[str, Any], scenarios: list[dict[str, Any]]) -> dict[str, Any]:
    """Are "how much tension" and "how costly to adjust" separable quantities?

    ``commitment`` is a term in the tension index (weight 0.25) *and* the largest
    term in adjustment cost (0.45); ``volition_self`` is a term in the index (0.20)
    *and* a term in adjustment cost (0.20). Two composites sharing two of their
    inputs are not obviously two constructs, and the design asserts they are. This
    measures the overlap on the corpus instead of assuming it away: a Pearson
    correlation near 1.0 would mean the index is largely a restatement of cost,
    and the "tension" reading would have to be reported as such.

    Returns the correlation plus the shared-input disclosure.
    """
    tensions: list[float] = []
    costs: list[float] = []
    for scenario in scenarios:
        ...
        try:
            ...
            continue
        tensions.append(float(evaluation["evaluation_result"]["tension"]))
        costs.append(float(evaluation["evaluation_result"]["adjustment_cost"]))

    shared = sorted(
        set(config["index"]["weights"]) & set(config["evaluator"]["adjustment_cost_weights"])
    )
    if len(tensions) < 3:
        return {"n": len(tensions), "pearson": None, "shared_inputs": shared}

    mean_t = sum(tensions) / len(tensions)
    mean_c = sum(costs) / len(costs)
    cov = sum((t - mean_t) * (c - mean_c) for t, c in zip(tensions, costs))
    var_t = sum((t - mean_t) ** 2 for t in tensions)
    var_c = sum((c - mean_c) ** 2 for c in costs)
    denominator = (var_t * var_c) ** 0.5
    pearson = cov / denominator if denominator > 0 else None
    return {"n": len(tensions), "pearson": pearson, "shared_inputs": shared}
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.write_report:
        discriminant = discriminant_check(config, scenarios)
        lines = [
        ...
        if proximity["n"]:
            lines += [
                f"- routed cases: {proximity['n']}",
                f"- `e_score` boundaries in the rule list: {proximity['boundaries']}",
                f"- nearest boundary, min / median: {proximity['min']:.4f} / {proximity['median']:.4f}",
                f"- cases within 0.05 of a boundary: {proximity['within_delta']}",
                "",
            ]
            if proximity["median"] > 0.10:
                lines += [
                    "The corpus sits comfortably inside its bands, so the zero flip rate",
                    "reflects corpus design as much as routing robustness.",
                    "",
                ]
            else:
                lines += [
            ...
            lines += ["No routed cases to measure.", ""]
        ...
        lines += [
            ...
            "",
            f"- shared inputs: {discriminant['shared_inputs'] or 'none'}",
        ]
        if discriminant["pearson"] is None:
            lines += ["- Pearson r over routed cases: not computable (too few cases)", ""]
        else:
            lines += [
                f"- Pearson r (tension, adjustment_cost) over {discriminant['n']} routed cases: "
                f"{discriminant['pearson']:.3f}",
                "",
            ]
            if abs(discriminant["pearson"]) >= 0.80:
                lines += [
